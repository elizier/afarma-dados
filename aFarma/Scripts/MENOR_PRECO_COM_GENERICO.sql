...
then (min((coalesce(mp.valor_1,0)*:quantidade1)+(coalesce(mp.valor_2,0)*:quantidade2)+(coalesce(mp.valor_3,0)*:quantidade3))-coalesce(:desconto,0))
when (count(mp.ean_1)<count(mp.concorrente) or count(mp.ean_2)<count(mp.concorrente) or count(mp.ean_3)<count(mp.concorrente))
then ((min((mp.valor_1)*:quantidade1)+min((mp.valor_2)*:quantidade2)+min((mp.valor_3)*:quantidade3))-coalesce(:desconto,0))
end)::numeric,2)
from 
(
select 
...
produtos.add(new ItemParaCotacaoDTO("7891142177162",2));
		produtos.add(new ItemParaCotacaoDTO("7896658008801",3));
		produtos.add(new ItemParaCotacaoDTO("7896112425410",1));
